...
dockerrmf () {
	#Cleanup demo2 containers on the host (dead or alive).
...
instruction "Welcome to Demo2 - SSVM Ethernet between containers.
...
instruction "This Demo will create two docker containers. Each running an instance of VPP within.
 ...
 A topology diagram is available in this repo under the 'Demo2' directory. "
...
docker exec ssvm-master /vagrant/demo2/install-vpp-container.sh;
docker exec ssvm-slave /vagrant/demo2/install-vpp-container.sh
"
...
echo MASTER CONFIG: ;
cat /vagrant/demo2/vpe-ssvm-master.conf;
cp /vagrant/demo2/vpe-ssvm-master.conf /home/vagrant/vpe-ssvm-master.conf;
echo SLAVE CONFIG: ;
cat /vagrant/demo2/vpe-ssvm-slave.conf;
cp /vagrant/demo2/vpe-ssvm-slave.conf /home/vagrant/vpe-ssvm-slave.conf;
"
pause

instruction "Start our VPP Instances" "
docker exec ssvm-master /vagrant/demo2/start-vpp-master.sh;
docker exec ssvm-slave /vagrant/demo2/start-vpp-slave.sh;
...
ip netns exec ssvm-master ip addr add 192.168.1.2/24 dev foobar;
...
ip netns exec ssvm-slave ip addr add 192.168.2.2/24 dev foobar;
...
instruction "Thanks it for demo2. all commands and scripts are within " " # /vagrant/demo2.sh"
